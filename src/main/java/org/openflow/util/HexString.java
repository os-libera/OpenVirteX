...
public class HexString {
    ...
    public static String toHexString(final long val, final int padTo) {
        final char arr[] = Long.toHexString(val).toCharArray();
        String ret = "";
    ...
    public static byte[] fromHexString(final String values)
            throws NumberFormatException {
        final String[] octets = values.split(":");
        final byte[] ret = new byte[octets.length];
    ...
    public static long toLong(final String values) throws NumberFormatException {
        ...
        final BigInteger bi = new BigInteger(values.replaceAll(":", ""), 16);
        if (bi.bitLength() > 64) {
