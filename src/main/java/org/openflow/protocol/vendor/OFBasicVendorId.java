...
public class OFBasicVendorId extends OFVendorId {
     ...
     * OFBasicVendorId assumes that this value is common across all of the
     * vendor data formats associated with a given vendor id.
     *
     * @return
     */
     ...
     * determine the appropriate OFVendorDataType for the data. It is meant to
     * be a reasonably generic implementation that will work for most formats of
     * vendor extensions. If the vendor data doesn't fit the assumptions listed
     * below, then this method will need to be overridden to implement custom
     * parsing.
     *
     * This implementation assumes that the vendor data begins with a data type
     * code that is used to distinguish different formats of vendor data
     * associated with a particular vendor ID. The exact format of the data is
     * vendor-defined, so we don't know how how big the code is (or really even
     * if there is a code). This code assumes that the common case will be that
     * the data does include an initial type code (i.e. so that the vendor can
     * have multiple message/data types) and that the size is either 1, 2 or 4
     * bytes. The size of the initial type code is configured by the subclass of
     * OFVendorId.
     *
     * @param data
     *            the channel buffer containing the vendor data.
     * @param length
