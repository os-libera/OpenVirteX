...
/**
 * The singleton class database manager is responsible for all read/write
 * operations to persistent storage. Upon start, it will read all persisted
 * virtual networks, and spawn virtual network managers for each that will wait
 * until all their elements are available and then boot the virtual network.
 */
public final class DBManager {
    ...
    private final Map<Long, List<OVXNetworkManager>> dpidToMngr;
    // Mapping between physical links and a list of vnet managers
    private final Map<DPIDandPortPair, List<OVXNetworkManager>> linkToMngr;
    // Mapping between physical ports and a list of vnet managers
    private final Map<DPIDandPort, List<OVXNetworkManager>> portToMngr;

    private static Logger log = LogManager.getLogger(DBManager.class.getName());

    /**
     * Creates the database manager instance. Connects to the database backend,
     * and creates mappings between network elements and virtual network
     * managers.
     */
    private DBManager() {
    ...
    /**
     * Gets the database manager instance, and creates one if it doesn't exist.
     *
    ...
    /**
     * Initializes database backend by creating config, user and vnet
     * collections.
     *
     * @param host
     *            the database host
     ...
     *            the database port
     * @param clear
     *            flag to clear the database
     */
    public void init(final String host, final Integer port, final boolean clear) {
        ...
        try {
            ...
            final DB db = ((MongoConnection) this.dbConnection).getDB();

            final DBCollection cfg = db.getCollection(DBManager.DB_CONFIG);
            this.collections.put(DBManager.DB_CONFIG, cfg);
            final DBCollection user = db.getCollection(DBManager.DB_USER);
            this.collections.put(DBManager.DB_USER, user);
            final DBCollection vnet = db.getCollection(DBManager.DB_VNET);
            ...
            DBManager.log.error("Failed to initialize database: {}",
					e.getMessage());
        } finally {
     ...
     *            the collection
     */
    private void setIndex(final String coll) {
        // Suppress error stream when MongoDB raises java.net.ConnectException
        // in another component (and cannot be caught)
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            final BasicDBObject options = new BasicDBObject("unique", true);
            final BasicDBObject index = new BasicDBObject(TenantHandler.TENANT,
					1);
            this.collections.get(coll).ensureIndex(index, options);
        } catch (final Exception e) {
            DBManager.log.error("Failed to set database index: {}",
					e.getMessage());
        } finally {
            // Restore error stream
            System.setErr(ps);
     ...
     *            the collection
     */
    private void clear(final String coll) {
        // Suppress error stream when MongoDB raises java.net.ConnectException
        // in another component (and cannot be caught)
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            ...
            DBManager.log.error("Failed to clear database: {}", e.getMessage());
        } finally {
            // Restore error stream
            System.setErr(ps);
        }
    ...
    public void close() {
        ...
        try {
            ...
            DBManager.log.error("Failed to close database connection: {}",
					e.getMessage());
        } finally {
            // Restore error stream
            System.setErr(ps);
     ...
     */
    public void createDoc(final Persistable obj) {
        // Suppress error stream when MongoDB raises java.net.ConnectException
        // in another component (and cannot be caught)
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            final DBCollection collection = this.collections.get(obj
					.getDBName());
            ...
            if (e instanceof MongoException.DuplicateKey) {
                DBManager.log
						.warn("Skipped saving of virtual network with duplicate tenant id");
            } else {
                DBManager.log.error(
     ...
     */
    public void removeDoc(final Persistable obj) {
        // Suppress error stream when MongoDB raises java.net.ConnectException
        // in another component (and cannot be caught)
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            ...
            DBManager.log.error("Failed to remove document from database: {}",
                    e.getMessage());
        } finally {
            // Restore error stream
            System.setErr(ps);
     ...
     */
    public void save(final Persistable obj) {
        final BasicDBObject query = new BasicDBObject();
        query.putAll(obj.getDBIndex());
        final BasicDBObject update = new BasicDBObject("$addToSet",
     ...
     *            the object to remove
     */
    public void remove(final Persistable obj) {
        final BasicDBObject query = new BasicDBObject();
        query.putAll(obj.getDBIndex());
        final BasicDBObject update = new BasicDBObject("$pull",
				new BasicDBObject(obj.getDBKey(), obj.getDBObject()));
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            ...
            DBManager.log.error("Failed to remove from db: {}", e.getMessage());
        } finally {
            System.setErr(ps);
        }
    }
     ...
     *            the dpid
     */
    public void removeSwitchPath(final int tenantId, final long switchId) {
        final BasicDBObject query = new BasicDBObject();
        query.put(TenantHandler.TENANT, tenantId);
        final BasicDBObject pull = new BasicDBObject("$pull",
				new BasicDBObject(SwitchRoute.DB_KEY, new BasicDBObject(
						TenantHandler.DPID, switchId)));
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            final DBCollection collection = this.collections
					.get(DBManager.DB_VNET);
     ...
     *            the link ID
     */
    public void removeLinkPath(final int tenantId, final int linkId) {
        final BasicDBObject query = new BasicDBObject();
        query.put(TenantHandler.TENANT, tenantId);
        final BasicDBObject pull = new BasicDBObject("$pull",
				new BasicDBObject(Link.DB_KEY, new BasicDBObject(
						TenantHandler.LINK, linkId)));
        final PrintStream ps = System.err;
        System.setErr(null);
        try {
            final DBCollection collection = this.collections
    ...
    private void readOVXNetworks() {
        ...
        try {
            ...
            final DBCollection coll = this.collections.get(DBManager.DB_VNET);
            final DBCursor cursor = coll.find();
            DBManager.log.info("Loading {} virtual networks from database",
					cursor.size());
            while (cursor.hasNext()) {
                OVXNetworkManager mngr = null;
                final Map<String, Object> vnet = cursor.next().toMap();
                try {
                    ...
                    // abstract OVXSwitch
                    final List<Map<String, Object>> switches = (List<Map<String, Object>>) vnet
                            .get(Switch.DB_KEY);
                    final List<Map<String, Object>> links = (List<Map<String, Object>>) vnet
                            .get(Link.DB_KEY);
                    final List<Map<String, Object>> ports = (List<Map<String, Object>>) vnet
                            .get(Port.DB_KEY);
                    final List<Map<String, Object>> routes = (List<Map<String, Object>>) vnet
            ...
            DBManager.log.error("Failed to load virtual networks from db: {}",
                    e.getMessage());
        } finally {
            System.setErr(ps);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private void readOVXSwitches(final List<Map<String, Object>> switches,
            final OVXNetworkManager mngr) {
        if (switches == null) {
            return;
        }
        // Read explicit switch mappings (virtual to physical)
        for (final Map<String, Object> sw : switches) {
            final List<Long> physwitches = (List<Long>) sw
					.get(TenantHandler.DPIDS);
            for (final Long physwitch : physwitches) {
                mngr.registerSwitch(physwitch);
                final List<OVXNetworkManager> mngrs = this.dpidToMngr
						.get(physwitch);
                if (mngrs == null) {
     ...
     */
    @SuppressWarnings("unchecked")
    private void readOVXLinks(final List<Map<String, Object>> links,
            final OVXNetworkManager mngr) {
        if (links == null) {
            return;
        }
        ...
        for (final Map<String, Object> link : links) {
            final List<Map<String, Object>> path = (List<Map<String, Object>>) link
                    .get(TenantHandler.PATH);
            for (final Map<String, Object> hop : path) {
                // Fetch link
                final Long srcDpid = (Long) hop.get(TenantHandler.SRC_DPID);
                final Short srcPort = ((Integer) hop
						.get(TenantHandler.SRC_PORT)).shortValue();
                final Long dstDpid = (Long) hop.get(TenantHandler.DST_DPID);
                final Short dstPort = ((Integer) hop
						.get(TenantHandler.DST_PORT)).shortValue();
                final DPIDandPortPair dpp = new DPIDandPortPair(
						new DPIDandPort(srcDpid, srcPort), new DPIDandPort(
								dstDpid, dstPort));
     ...
     *            the virtual network manager
     */
    private void readOVXPorts(final List<Map<String, Object>> ports,
            final OVXNetworkManager mngr) {
        if (ports == null) {
            return;
        }
        for (final Map<String, Object> port : ports) {
            // Read dpid and port number
            final Long dpid = (Long) port.get(TenantHandler.DPID);
            final Short portNumber = ((Integer) port.get(TenantHandler.PORT))
                    .shortValue();
            final DPIDandPort p = new DPIDandPort(dpid, portNumber);
            // Register port in current manager
            mngr.registerPort(p);
            // Update list of managers that wait for this port
            final List<OVXNetworkManager> mngrs = this.portToMngr.get(p);
            if (mngrs == null) {
     ...
     */
    @SuppressWarnings({ "unchecked" })
    private void readOVXRoutes(final List<Map<String, Object>> routes,
            final OVXNetworkManager mngr) {
        if (routes == null) {
            return;
        }
        for (final Map<String, Object> route : routes) {
            final List<Map<String, Object>> path = (List<Map<String, Object>>) route
                    .get(TenantHandler.PATH);
            for (final Map<String, Object> hop : path) {
                final Long srcDpid = (Long) hop.get(TenantHandler.SRC_DPID);
                final Short srcPort = ((Integer) hop
    ...
    public void addSwitch(final Long dpid) {
        ...
        if (!this.clear) {
            final List<OVXNetworkManager> completedMngrs = new ArrayList<OVXNetworkManager>();
            synchronized (this.dpidToMngr) {
                // Lookup virtual networks that use this physical switch
                final List<OVXNetworkManager> mngrs = this.dpidToMngr.get(dpid);
                if (mngrs != null) {
                    for (final OVXNetworkManager mngr : mngrs) {
                        mngr.setSwitch(dpid);
                        if (mngr.getStatus()) {
     ...
     *
     * @param dpp
     *            physical link given as a dpid and port pair
     */
    public void addLink(final DPIDandPortPair dpp) {
        ...
        if (!this.clear) {
            ...
            synchronized (this.linkToMngr) {
                ...
                final List<OVXNetworkManager> mngrs = this.linkToMngr.get(dpp);
                if (mngrs != null) {
                    for (final OVXNetworkManager mngr : mngrs) {
                        mngr.setLink(dpp);
                        if (mngr.getStatus()) {
     ...
     *
     * @param dpp
     *            the physical link given as a dpid and port pair
     */
     ...
     *
     * @param port
     *            the port given as a dpid and port pair
     */
    public void addPort(final DPIDandPort port) {
        ...
        if (!this.clear) {
            ...
            synchronized (this.portToMngr) {
                ...
                final List<OVXNetworkManager> mngrs = this.portToMngr.get(port);
                if (mngrs != null) {
                    for (final OVXNetworkManager mngr : mngrs) {
                        mngr.setPort(port);
                        if (mngr.getStatus()) {
     ...
     *
     * @param mngrs
     *            list of virtual network managers
     */
    private void removeOVXNetworkManagers(final List<OVXNetworkManager> mngrs) {
        for (final OVXNetworkManager mngr : mngrs) {
            synchronized (this.dpidToMngr) {
                for (final Long dpid : this.dpidToMngr.keySet()) {
                    this.dpidToMngr.get(dpid).remove(mngr);
                }
            }
            synchronized (this.linkToMngr) {
                for (final DPIDandPortPair dpp : this.linkToMngr.keySet()) {
                    this.linkToMngr.get(dpp).remove(mngr);
                }
            }
            synchronized (this.portToMngr) {
                for (final DPIDandPort dp : this.portToMngr.keySet()) {
