...
public class OVXError extends OFError implements Virtualizable, Devirtualizable {
    ...
    public void virtualize(final PhysicalSwitch sw) {
         ...
         */
        this.log.error(OVXError.getErrorString(this));
     ...
     */
    private static String getErrorString(final OFError error) {
        // TODO: this really should be OFError.toString. Sigh.
        final int etint = 0xffff & error.getErrorType();
        if (etint < 0 || etint >= OFErrorType.values().length) {
            return String.format("Unknown error type %d", etint);
        }
        final OFErrorType et = OFErrorType.values()[etint];
        switch (et) {
            case OFPET_HELLO_FAILED:
                final OFHelloFailedCode hfc = OFHelloFailedCode.values()[0xffff & error
                        .getErrorCode()];
                return String.format("Error %s %s", et, hfc);
            case OFPET_BAD_REQUEST:
                final OFBadRequestCode brc = OFBadRequestCode.values()[0xffff & error
                        .getErrorCode()];
                return String.format("Error %s %s", et, brc);
            case OFPET_BAD_ACTION:
                final OFBadActionCode bac = OFBadActionCode.values()[0xffff & error
                        .getErrorCode()];
                return String.format("Error %s %s", et, bac);
            case OFPET_FLOW_MOD_FAILED:
                final OFFlowModFailedCode fmfc = OFFlowModFailedCode.values()[0xffff & error
                        .getErrorCode()];
                return String.format("Error %s %s", et, fmfc);
            case OFPET_PORT_MOD_FAILED:
                final OFPortModFailedCode pmfc = OFPortModFailedCode.values()[0xffff & error
                        .getErrorCode()];
                return String.format("Error %s %s", et, pmfc);
            case OFPET_QUEUE_OP_FAILED:
                final OFQueueOpFailedCode qofc = OFQueueOpFailedCode.values()[0xffff & error
                        .getErrorCode()];
