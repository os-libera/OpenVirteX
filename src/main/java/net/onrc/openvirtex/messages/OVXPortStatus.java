...
/**
 * Handler for PortStatus messages from the network. From the virtual side,
 * HOW the port was disabled in the network doesn't matter, so only 
 * a PhysicalPort is duly affected by state/configuration changes in the message.  
 */
public class OVXPortStatus extends OFPortStatus implements Virtualizable {
	...
	public void virtualize(final PhysicalSwitch sw) { 
		log.info("Received {} from switch {} for port {}", this.toString(), sw.getSwitchId()
				, sw.getPort(this.desc.getPortNumber()) == null ? "null" :
					sw.getPort(this.desc.getPortNumber()).toAP());
