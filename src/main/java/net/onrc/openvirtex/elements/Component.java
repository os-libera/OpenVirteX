...
/**
 * Methods implemented by classes that represent network elements. Each component
 * is assumed to implement a state machine with the following states:
 * <ul>
 * <li> INIT - just initialized, not accessible or usable</li>
 * <li> ACTIVE - capable of handling events and accumulating state
 * (flow entries, counters), etc.</li>
 * <li> INACTIVE - halted, not capable of handling events or accumulating
 * state (e.g. a downed interface) </li>
 ...
 * with their own. In general, a subcomponent's state won't affect the component,
 * but the reverse is not true e.g. ports are removed if a switch is removed, but
 * a switch port can be removed without affecting the whole switch.
 */
public interface Component {
	 ...
	 * subcomponents. If this component is a Physical entity, it will also attempt
	 * to tearDown() Virtual components mapped to it.
	 *
