...
public class ShortestPath implements Routable {
    ...
    public void setLinkPath(final OVXLink ovxLink) throws PortMappingException {
        ...
        if (PhysicalNetwork.getInstance().getLink(
            ...
            if (srcPathPort.getParentSwitch() == dstPathPort.getParentSwitch()) {
                path.add(PhysicalNetwork.getInstance().getLink(
                ...
                for (Short i = 0; i <= U8.f(ovxLink.getAlg().getBackups()); i++) {
                    /*
                     * Remove from the physical network the shortest-paths
                     * already
                     * used. For the primary path, all the physical network is
                     * used
                     */
                    this.edges.removeAll(path);
                    path.clear();
                    ...
                    if (path == null) {
                        if (i == 0) {
                            ShortestPath.log
                                    .warn("Unable to compute the PRIMARY path for for link {} "
                                            ...
                                            ovxLink.getLinkId(), ovxLink
                                                    .getSrcSwitch()
                                                    .getSwitchName(), ovxLink
                                                    .getSrcPort()
                        ...
                        path.addFirst(PhysicalNetwork.getInstance().getLink(
                                ovxLink.getSrcPort().getPhysicalPort(),
                                srcPathPort));
                        path.add(PhysicalNetwork.getInstance().getLink(
                                dstPathPort,
