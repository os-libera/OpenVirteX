...
public class OVXLLDP extends LLDP {
    ...
    public static final byte[]  LLDP_NICIRA         = { 0x01, 0x23, 0x20, 0x00,
            0x00, 0x01                             };
    public static final byte[]  LLDP_MULTICAST      = { 0x01, (byte) 0x80,
            (byte) 0xc2, 0x00, 0x00, 0x0e          };
    public static final byte[]  BDDP_MULTICAST      = { (byte) 0xff,
    ...
    private static final byte[] NAME_TLV            = ByteBuffer
                                                            .allocate(
                                                                    OVXLLDP.NAME_TLV_SIZE + 2)
                                                            .putShort(
                                                                    OVXLLDP.NAME_TLV_HEADER)
                                                            .put(OVXLLDP.ONLAB_OUI)
                                                            .put(OVXLLDP.NAME_TLV_SUBTYPE)
                                                            .put(OVXLLDP.OVX_NAME
                                                                    .getBytes())
                                                            .array();

    private static final byte   DPID_TLV_TYPE       = 127;
    private static final byte   DPID_TLV_SIZE       = (byte) 12;                                                   // 12
                                                                                                                    // =
                                                                                                                    // OUI
                                                                                                                    // (3)
                                                                                                                    // +
    ...
    private static final byte[] DPID_TLV            = ByteBuffer
                                                            .allocate(
                                                                    OVXLLDP.DPID_TLV_SIZE + 2 - 8)
                                                            .putShort(
                                                                    OVXLLDP.DPID_TLV_HEADER)
                                                            .put(OVXLLDP.ONLAB_OUI)
                                                            .put(OVXLLDP.DPID_TLV_SUBTYPE)
                                                            .array();

    // Pre-built contents of both organizationally specific TLVs
    private static final byte[] OUI_TLV             = ArrayUtils.addAll(
                                                            OVXLLDP.NAME_TLV,
                                                            OVXLLDP.DPID_TLV);

    // Default switch, port number and TTL
    private static final byte[] DEFAULT_DPID        = { 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00                 };
    private static final short  DEFAULT_PORT        = 0;
    private static final short  DEFAULT_TTL         = 120;                                                         // in
                                                                                                                    // seconds
    ...
    private static final short  OVX_LLDP_SIZE       = (short) (OVXLLDP.CHASSIS_TLV_SIZE
                                                            ...
                                                            + OVXLLDP.NAME_TLV_SIZE
                                                            + OVXLLDP.DPID_TLV_SIZE + 12);
