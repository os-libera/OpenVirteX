 ...
 * fast. Every fmRate milliseconds, loop over all fast ports and send an
 ...
 */
public class OVXLinkManager implements OVXSendMsg,
        TimerTask {
    ...
    Logger                                  log               = LogManager
	                                                              .getLogger(OVXLinkManager.class
	                                                                      .getName());
    ...
    public void registerOVXPort(OVXPort ovxPort) {
	HashMap<PhysicalSwitch,OVXFlowMod> portMap = new HashMap<PhysicalSwitch, OVXFlowMod>();
	...
	fm.setActions(Arrays.asList((OFAction) new OFActionOutput(OFPort.OFPP_CONTROLLER.getValue(), (short) 0xffff)));
	...
	System.out.println("resistro su questo switch e porta " + ovxPort.getPhysicalPort().getParentSwitch().getName() + " "+ ovxPort.getPhysicalPortNumber() + " "+ fm.toString());
	this.portFlowMap.put(ovxPort, portMap);
    }
    
    
    public boolean registerOVXLink(OVXLink ovxLink) {
    	Short inPort = 0;
	...
	for (PhysicalLink phyLink : this.map.getPhysicalLinks(ovxLink)) {
	    if (inPort != 0) {
		/*
		 * generate the flowMod, using the previous physical link dst port id as input port, 
		 * and this physical link src port as output port
    ...
    public boolean unregisterOVXLink(OVXLink ovxLink) {
    	...
    	if (linkMap.isEmpty())
    		...
    		for (PhysicalSwitch phySwitch : linkMap.keySet()) {
    			OVXFlowMod fm = linkMap.get(phySwitch);
    			fm.setCommand(OVXFlowMod.OFPFC_DELETE);
    			fm.setFlags(OVXFlowMod.OFPFF_SEND_FLOW_REM); // verify if we want the flow-rem message from the switch
    			//fm.setOutPort(port);
    ...
    public void start(){
	PhysicalNetwork.getTimer().newTimeout(this, 10,
	        TimeUnit.MILLISECONDS);
	this.log.debug("Starting virtual link manager for virtual network {}", this.tenantId);
    }
    
    /**
     * Execute this method every fmRate milliseconds. Loops over all physical switches 
     * refreshing the proactive flow-mods that describe the virtual links
     * 
     * @param t
     * @throws Exception
     */
	...
	this.log.debug("Sending virtual links proactive flow-mods for virtual network {}", this.tenantId);

	synchronized (this) {
		for (OVXLink ovxLink : this.flowMap.keySet()) {
			for (PhysicalSwitch phySwitch : this.flowMap.get(ovxLink).keySet()) {
			    this.log.trace("Sending flow-mod to sw {} , {}", phySwitch.getName(), this.flowMap.get(ovxLink).get(phySwitch).toString());
			    phySwitch.sendMsg(this.flowMap.get(ovxLink).get(phySwitch), this);
			}		
		}
		
		for (OVXPort ovxPort : this.portFlowMap.keySet()) {
